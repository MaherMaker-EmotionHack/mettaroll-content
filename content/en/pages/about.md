---
title: About Us
hero_headline: ABOUT METTAROLL
hero_image: /uploads/codex-bild-20.-aug.-2026-21_47_59.png
vision:
  heading: Our Vision
  body: Our vision is to become a recognized international steel trading group,
    building long-term bridges between global steel suppliers and high-growth
    markets.
story:
  heading: Our Story
  body: >-
    Mettaroll was built on international steel trade experience, direct market
    knowledge, and a clear ambition to connect reliable steel supply with
    high-growth markets.


    Our strength comes from understanding both sides of the trade: the European supply market and the international destination markets where steel is needed, purchased, and used. This market connection allows us to support customers with stock availability, sourcing, trade coordination, and export-focused solutions.


    Today, Mettaroll is focused on building strong relationships with customers, suppliers, stockholders, and logistics partners across global steel markets.
  image: /uploads/codex-bild-20.-aug.-2026-21_17_45.png
values:
  heading: What We Stand For
  items:
    - title: Trust & Integrity
      description: Transparent communication, honest conduct, and reliable commitments
        are the foundation of every trade.
      icon: shield
    - title: Market Intelligence
      description: We combine supplier knowledge with direct experience in
        international destination markets.
      icon: truck
    - title: Operational Reliability
      description: We focus on structured processes, clear coordination, and
        consistent execution.
      icon: wrench
    - title: Commercial Flexibility
      description: We adapt to different product needs, trade structures, markets, and
        customer requirements.
    - title: Strategic Partnerships
      description: We build strong relationships with customers, suppliers,
        stockholders, and logistics partners to create sustainable value across
        markets.
    - title: Export Excellence
      description: We support international steel trade with export-focused
        coordination, market understanding, and practical execution.
stats:
  - value: Rotterdam
    label: Company Base
  - value: Antwerp
    label: Stock & Warehousing
  - value: B2B
    label: Steel Markets
  - value: Global
    label: Trade Focus
seo:
  meta_title: About Mettaroll | International Steel Trading Company
  meta_description: Learn more about Mettaroll, an international steel trading and
    export company based in Rotterdam, focused on steel supply, stock
    availability, market expertise, and global B2B trade.
---
